 ...
 *                                       Default true.
 *         @type bool $ratings           Whether to return the number of rating for each stars (1-5). Default true.
	...
	<div class="fyi">
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) { ?>
			...
			foreach ( $api->ratings as $key => $ratecount ) {
				...
				$aria_label = esc_attr( sprintf( _n( 'Reviews with %1$d stars: %2$s. Opens in a new window.', 'Reviews with %1$d stars: %2$s. Opens in a new window.', $key ),
					$key,
				...
				<div class="counter-container">
						<span class="counter-label"><a href="https://wordpress.org/support/view/plugin-reviews/<?php echo $api->slug; ?>?filter=<?php echo $key; ?>"
						                               target="_blank" aria-label="<?php echo $aria_label; ?>"><?php printf( _n( '%d stars', '%d stars', $key ), $key ); ?></a></span>
